...
namespace Silverlake.Repo
{
    public class UserRoleRepo : IUserRoleRepo
    {
        ...
        public UserRole PostData(UserRole obj)
        {
            try
            {
                string query = obj.ObjectToQuery<UserRole>("insert") + "SELECT SCOPE_IDENTITY()";
        ...
        public Int32 PostBulkData(List<UserRole> objs)
        {
            ...
            try
            {
                ...
                objs.ForEach(obj =>
                {
                    query += obj.ObjectToQuery<UserRole>("insert") + "SELECT SCOPE_IDENTITY()";
                });
                SqlCommand cmd = new SqlCommand(query, mySQLDBConnect.connection);
                mySQLDBConnect.OpenConnection();
        ...
        public UserRole UpdateData(UserRole obj)
        {
            try
            {
                string query = obj.ObjectToQuery<UserRole>("update");
                SqlCommand cmd = new SqlCommand(query, mySQLDBConnect.connection);
                mySQLDBConnect.OpenConnection();
                cmd.ExecuteNonQuery();
                mySQLDBConnect.CloseConnection();
        ...
        public Int32 UpdateBulkData(List<UserRole> objs)
        {
            ...
            try
            {
                ...
                objs.ForEach(obj =>
                {
                    query += obj.ObjectToQuery<UserRole>("update");
                });
                SqlCommand cmd = new SqlCommand(query, mySQLDBConnect.connection);
                mySQLDBConnect.OpenConnection();
                result = cmd.ExecuteNonQuery();
            ...
            try
            {
                ...
                obj = (from x in dt.AsEnumerable()
                        select new UserRole
                        {
                            ...
                            UserTypeId = x.Field<Int32>("user_type_id"),
        ...
        {
            string columnName = Converter.GetColumnNameByPropertyName<UserRole>(propertyName);
            List<UserRole> objs = new List<UserRole>();
            try
            {
                string query = "select * from user_roles where " + columnName + " = '" + propertyValue + "'";
                if(!isEqual)
                    query = "select * from user_roles where " + columnName + " != '" + propertyValue + "'";
                if (isOrderByDesc)
                    query += " order by ID desc";
                else
                    query += " order by ID";
        ...
        public Int32 GetCountByPropertyName(string propertyName, string propertyValue, bool isEqual)
        {
            ...
            try
            {
                string query = "select count(*) from user_roles where " + columnName + " = '" + propertyValue + "'";
                if(!isEqual)
                    query = "select count(*) from user_roles where " + columnName + " != '" + propertyValue + "'";
                SqlCommand cmd = new SqlCommand(query, mySQLDBConnect.connection);
                mySQLDBConnect.OpenConnection();
                count = Convert.ToInt32(cmd.ExecuteScalar());
                mySQLDBConnect.CloseConnection();
        ...
        public List<UserRole> GetDataByFilter(string filter, int skip, int take, bool isOrderByDesc)
        {
            ...
            try
            {
                string query = "select * from user_roles where " + filter +"";
                if (isOrderByDesc)
                    query += " order by ID desc";
                else
                    query += " order by ID";
        ...
        public Int32 GetCountByFilter(string filter)
        {
            ...
            try
            {
                string query = "select count(*) from user_roles where " + filter;
                SqlCommand cmd = new SqlCommand(query, mySQLDBConnect.connection);
                mySQLDBConnect.OpenConnection();
                count = Convert.ToInt32(cmd.ExecuteScalar());
                mySQLDBConnect.CloseConnection();
        ...
        public List<UserRole> GetDataByPaging(int take, int skip, out int filteredResultsCount, out int totalResultsCount)
        {
            ...
            try
            {
                string query = "select * from user_roles limit " + skip + ", " + take;
                SqlCommand cmd = new SqlCommand(query, mySQLDBConnect.connection);
                mySQLDBConnect.OpenConnection();
                DataTable dt = new DataTable();
                SqlDataAdapter dA = new SqlDataAdapter(cmd);
